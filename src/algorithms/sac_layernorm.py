...
class SoftQNetwork(nn.Module):
    def __init__(self, env, hidden_size):
        ...
        # LayerNorm placed AFTER the linear, BEFORE the activation (RLPD / jaxrl5 ordering).
        # This is the primary fix for critic value over-extrapolation (Q-value collapse /
        # Q-loss explosion). Ball, Smith, Kostrikov, Levine 2023, "Efficient Online RL with
        # Offline Data" (RLPD).
...
class SAC:
    def __init__(self, cfg, env):
        ...
        self.q_lr = cfg.q_lr
        # Max global-norm for critic gradient clipping. Suppresses the Q-loss spikes that
        # accompany value divergence. Move to cfg if you want Optuna to tune it; 10.0 is a
        # gentle default that rarely hurts a healthy run.
    ...
    def update(self, data): # where data is a ReplayBuffer with torch.tensor type
        ...
        q_loss.backward()
        # Clip critic gradients (global norm) before the step to damp Q-loss explosions.
        ...
        metrics = {
            ...
            "losses/q_grad_norm": q_grad_norm.item(),
